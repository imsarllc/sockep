...
int main()
{
	...
	auto srvr = sockep::SockEPFactory::createUnixStreamServerSockEP("/tmp/unixstreamserver", messageHandler);
	srvr->startServer();
	client = sockep::SockEPFactory::createUnixStreamClientSockEP("/tmp/unixstreamclient", "/tmp/unixstreamserver");
